    ...
    <header className="sticky top-0 z-50 backdrop-blur bg-black/30 border-b border-white/10">
      <div className="max-w-auto mx-auto px-4 py-3 flex items-center justify-between bg-white">
        ...
        <nav className="hidden md:flex gap-6 text-sm">
          <Link to="/services" className="nav-link mt-3">{t('nav.services')}</Link>
          <Link to="/equipment" className="nav-link mt-3">{t('nav.equipment')}</Link>
          <Link to="/about" className="nav-link mt-3">{t('nav.about')}</Link>
